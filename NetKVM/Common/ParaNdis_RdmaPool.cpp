...
NTSTATUS ParaNdis_RdmaPoolConnect(PARANDIS_ADAPTER *pContext)
{
    ...
    DPrintf(0, "Connected to rdmapool VA=%p PA=0x%llx Size=0x%llx",
            pContext->RdmaPoolBaseVA, pContext->RdmaPoolBasePA.QuadPart, pContext->RdmaPoolSize);
...
PVOID ParaNdis_RdmaPoolAllocate(PARANDIS_ADAPTER *pContext, ULONG size, PHYSICAL_ADDRESS *pPa)
{
    ...
    DPrintf(2, "rdmapool alloc VA=%p PA=0x%llx size=0x%x",
            allocOutput.VirtualAddress, allocOutput.PhysicalAddress.QuadPart, size);
